			...
			<div class="card card-primary">
				<div class="card-body">
					<form id="${tabla}" class="needs-validation" novalidate>
						...
						<div class="card-header tx-medium bd-0 tx-white bg-primary-gradient"><i class="las la-city"></i> MI SUCURSAL</div>
					...
					listado+=`<tr id="${resp[i].ID_SUCURSAL}">
									<td>
										<div class="estadoTachado nombre muestraMensaje">${resp[i].NOMB_SUCURSAL }</div>
									...
									<td>
										<div class="estadoTachado celular">${resp[i].NRO_CELULAR}</div>
									</td>
									<td>
										<div class="estadoTachado documentos">${(resp[i].DOCUMENTOS===null)?'Ilimitado':resp[i].DOCUMENTOS }</div>
									</td>
									<td>
										<div class="estadoTachado documentos_emitidos">${resp[i].DOCUMENTOS_GENERADOS}</div>
									</td>
									<td>
										${estado()+((resp[i].ES_PRINCIPAL==1)?'':modifica())}
									</td>
								</tr>`;
							}}
					listado+=`</tbody>
...
function enviaFormularioSucursal(objeto){
	if(objeto.id==0){
		...
		confirm("¡Modificará el registro: "+dato+"!",function(){
			return false;
		},async function(){
			bloquea();
			let body=fd;
