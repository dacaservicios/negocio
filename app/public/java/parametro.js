			...
			<div class="card card-primary">
				<div class="card-body">
					<form id="${tabla}" class="needs-validation" novalidate>
						...
						<div class="card-header tx-medium bd-0 tx-white bg-primary-gradient"><i class="la la-list-alt"></i> PARÁMETRO</div>
						<div class="row pt-3">
						...
						<div class="row">
							<div class="form-group col-md-12">
								...
								<input name="abreviatura" autocomplete="off" maxlength="10" type="text" class="form-control p-1" placeholder="Ingrese la abreviatura">
								<div class="vacio oculto">¡Campo obligatorio!</div>
								<div class="formato oculto">¡Formato Incorrecto!</div>
							</div> 
						</div>
					...
					listado+=`<tr id="${resp[i].ID_PARAMETRO}" >
									<td>
										<div class="estadoTachado descripcion muestraMensaje ${mestado} ${mvisible}">${resp[i].DESCRIPCION}</div>
									</td>
									<td>
										<div class="estadoTachado abreviatura ${mestado} ${mvisible}">${resp[i].ABREVIATURA}</div>
									</td>
									<td>
										${detalle()+estado()+visible()+modifica()+elimina()}
									</td>
								</tr>`;
								}
					listado+=`</tbody>
...
function eventosParametro(objeto){
	...
	$('#'+objeto.tabla+' div').on( 'click','button[name=btnGuarda]',function(){//guarda
		...
		validaFormularioParametro(objeto)
	});
	...
	$('#'+objeto.tabla+'Tabla tbody').on( 'click','td a.edita',function(){//edita
		...
		objeto.id=id;
		objeto.nombreEdit=descripcion;
		parametroEdita(objeto);
	});
	...
	$('#'+objeto.tabla+'Tabla tbody').on( 'click','td a.estado',function(){//estado
    	...
    	let id=evento.attr('id');
		let descripcion=evento.find("td div.descripcion").text();
		parametroEstado({id:id,descripcion:descripcion,tabla:objeto.tabla});
	});

	$('#'+objeto.tabla+'Tabla tbody').on( 'click','td a.visible',function(){//visible
		let evento=$(this).parents("tr")
    	let id=evento.attr('id');
		let descripcion=evento.find("td div.descripcion").text();
		parametroVisible({id:id,descripcion:descripcion,tabla:objeto.tabla});
	});
	...
	$('#'+objeto.tabla+'Tabla tbody').on( 'click','td a.elimina',function(){//elimina
    	...
    	let id=evento.attr('id');
		let descripcion=evento.find("td div.descripcion ").text();
		parametroElimina({id:id,descripcion:descripcion,tabla:objeto.tabla});
	});
...
async function parametroEdita(objeto){
	...
	objeto.abreviatura.val(resp.ABREVIATURA);
	objeto.descripcion.val(resp.DESCRIPCION);
}

function validaFormularioParametro(objeto){	
	validaVacio(objeto.descripcion);
	let vabrev=validaAbreviatura(objeto.abreviatura);

	if(objeto.descripcion.val()=="" || vabrev==false){
		return false;
	}else{
		enviaFormularioParametro(objeto);
	}
}

function enviaFormularioParametro(objeto){
		...
		try {
			...
			if(resp.resultado){
				if(objeto.id>0){
					...
					$("#"+objeto.tabla+"Tabla #"+objeto.id+" .abreviatura").text(resp.info.ABREVIATURA);
					...
					let rowNode =t.row.add( [
						...
						`<div class="estadoTachado abreviatura">${resp.info.ABREVIATURA}</div>`,
						detalle()+estado()+visible()+modifica()+elimina()
					] ).draw( false ).node();
					$( rowNode ).attr('id',resp.info.ID_PARAMETRO);
...
function parametroElimina(objeto){
	confirm("¡Eliminará el registro: "+objeto.descripcion+"!",function(){
		return false;
	},async function(){
        bloquea();
		try {
...
function parametroEstado(objeto){
	confirm("¡Cambiará el estado del registro: "+objeto.descripcion+"!",function(){
		return false;
	},async function(){
		bloquea();
		let body={
...
function parametroVisible(objeto){
	confirm("¡Cambiará la visibilidad del registro: "+objeto.descripcion+"!",function(){
		return false;
	},async function(){
		bloquea();
		let body={
		...
		try {
			const visible = await axios.put("/api/"+objeto.tabla+"/visible/"+objeto.id,body,{ 
				headers:{authorization: `Bearer ${verToken()}`} 
			});
			desbloquea();
			...
			if(resp.resultado){
				...
				$("#"+objeto.tabla+"Tabla #"+objeto.id+" .abreviatura").removeClass('opaco');
				$("#"+objeto.tabla+"Tabla #"+objeto.id+" .descripcion").removeClass('opaco');

				$("#"+objeto.tabla+"Tabla #"+objeto.id+" .abreviatura").addClass(visible);
				$("#"+objeto.tabla+"Tabla #"+objeto.id+" .descripcion").addClass(visible);
			...
			<div class="card card-primary">
				<div class="card-body">
					...
					<div class="table-responsive">
						...
						listado+=`<tr id="${resp[i].ID_PARAMETRO_DETALLE}">
									<td>
										<div class="estadoTachado descripcionDetalle muestraMensaje ${mestado} ${mvisible}">${resp[i].DESCRIPCIONDETALLE }</div>
									</td>
									<td>
										<div class="estadoTachado valorDetalle muestraMensaje ${mestado} ${mvisible}">${resp[i].VALORDETALLE }</div>
									</td>
									<td>
										<div class="estadoTachado abreviaturaDetalle ${mestado} ${mvisible}">${resp[i].ABREVIATURA }</div>
									</td>
									<td>
	...
	confirm("¡"+verbo+" el registro: "+dato+"!",function(){
		...
		try {
			...
			if(objeto.idDetalle==0){
				creaEdita = await axios.post("/api/"+objeto.tabla+"/detalle/crear",body,{ 
					headers:{
						authorization: `Bearer ${verToken()}`
					} 
				});
			}else{
				creaEdita = await axios.put("/api/"+objeto.tabla+"/detalle/editar/"+objeto.idDetalle,body,{ 
					headers:{
						authorization: `Bearer ${verToken()}`
					} 
				});
