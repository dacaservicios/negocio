...
async function vistaServicio(){
			...
			<div class="card card-primary">
				<div class="card-body">
					<form id="${tabla}" class="needs-validation" novalidate>
						...
						<div class="card-header tx-medium bd-0 tx-white bg-primary-gradient"><i class="las la-shopping-cart"></i> SERVICIO</div>
						<div class="row pt-3">
							...
							<div class="form-group col-md-5">
								...
								<select name="categoria" class="form-control select2">
									...
									for(var i=0;i<resp2.length;i++){
										if(resp2[i].ES_VIGENTE==1 && resp2[i].TIPO=='S'){
					...
					listado+=`<tr id="${resp[i].ID_SERVICIO}">
									<td>
	...
	eventosServicio(objeto);
}

function eventosServicio(objeto){
	...
	$('#'+objeto.tabla+' div').on( 'click','button[name=btnGuarda]',function(){//guarda
		...
		validaFormularioServicio(objeto)
	});
	...
	$('#'+objeto.tabla+'Tabla tbody').on( 'click','td a.estado',function(){//estado
    	...
    	let id=evento.attr('id');
		let nombre=evento.find("td div.nombre").text();
		servicioEstado({id:id,nombre:nombre,tabla:objeto.tabla});
	});
	...
	$('#'+objeto.tabla+'Tabla tbody').on( 'click','td a.elimina',function(){//elimina
    	...
    	let id=evento.attr('id');
		let nombre=evento.find("td div.nombre ").text();
		servicioElimina({id:id,nombre:nombre,tabla:objeto.tabla});
	});
}

async function servicioEdita(objeto){
...
function validaFormularioServicio(objeto){	
	...
	if(objeto.nombre.val()=="" || objeto.categoria.val()==""){
		...
		enviaFormularioServicio(objeto);
	}
}

function enviaFormularioServicio(objeto){
		...
		try {
			...
			if(resp.resultado){
				...
				if(objeto.id>0){
					...
					$( rowNode ).attr('id',resp.info.ID_SERVICIO);
...
function servicioElimina(objeto){
...
function servicioEstado(objeto){
