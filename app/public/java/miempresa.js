...
async function vistaEmpresa(){
	bloquea();
	let tabla="empresa";
	const lista= await axios.get('/api/'+tabla+'/listar/miempresa/0/'+verSesion(),{
		headers: 
		{ 
			...
			<div class="card card-primary">
				<div class="card-body">
					<form id="${tabla}" class="needs-validation" novalidate>
						...
						<div class="card-header tx-medium bd-0 tx-white bg-primary-gradient"><i class="las la-city"></i> MI EMPRESA</div>
						<div class="row pt-3">
						...
						<div class="row">
							<div class="form-group col-md-12">
								...
								<span id="imagenEmpresa" class="cursor"></span>
							</div>
	...
	$("#"+tabla+" span#botonGuardar").text('Modificar');
	$("#"+tabla+" button[name=btnLimpia]").attr('disabled','disabled');
	$("#"+tabla+" button[name=btnGuarda]").attr('disabled','disabled');
	$(".select2").select2({
		placeholder:'Select...',
	...
	$('#jsPropio').append("<script src='/java/misucursal.js?"+moment().format('DDMMYYYYHHmmss')+"'></script>");
}
...
function eventosEmpresa(objeto){
	...
	$('#'+objeto.tabla+' div').on( 'click','button[name=btnLimpia]',function(){//limpia
		limpiaTodoConfiguracion(objeto.tabla);
		$('span#imagenEmpresa').html('');
		$('#'+objeto.tabla+' button[name=btnLimpia]').attr('disabled','disabled');
		$('#'+objeto.tabla+' button[name=btnGuarda]').attr('disabled','disabled');
	});
...
async function empresaEdita(objeto){
	...
	if(resp.IMAGEN!==null){
		...
		$('span#imagenEmpresa').on( 'click',function(){
			let imagen=`<img src="../imagenes/sucursal/LOGO_`+resp.ID_EMPRESA+`_`+resp.IMAGEN+`">`;
			mostrar_general1({titulo:'IMAGEN',nombre:objeto.nombreEdit,msg:imagen,ancho:300});
			$('#contenidoGeneral1').addClass('text-center');
		});
	}
	$('#'+objeto.tabla+' button[name=btnLimpia]').removeAttr('disabled');
	$('#'+objeto.tabla+' button[name=btnGuarda]').removeAttr('disabled');
}
...
function enviaFormularioEmpresa(objeto){
	if(objeto.id==0){
		mensajeSistema('¡Usted no ha seleccionado ninguna empresa para modificar!');
	}else{
		let dato=objeto.nombreMsg;
		...
		confirm("¡Modificara el registro: "+dato+"!",function(){
			return false;
		},async function(){
			bloquea();
			let body=fd;
					...
					if(objeto.id>0){
						...
						if(resp.info.ID_EMPRESA==resp.info.EMPRESA_ACTUAL){
							if(resp.info.IMAGEN!==null){
								socket.emit('actualizaLogoSucursal',{
									id_sucursal: resp.info.ID_EMPRESA,
									imagen: resp.info.IMAGEN,
									usuario: "S"+$('#userSucursal').val()
								});
							}
