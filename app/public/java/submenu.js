...
async function vistaSubmenu(){
			...
			<div class="card card-primary">
				<div class="card-body">
					<form id="${tabla}" class="needs-validation" novalidate>
						...
						<div class="card-header tx-medium bd-0 tx-white bg-primary-gradient"><i class="la la-list"></i> SUBMENÚ</div>
						<div class="row pt-3">
							...
							<div class="form-group col-md-5">
								<label>Ruta (*)</label>
								<input name="ruta" autocomplete="off" maxlength="100" type="text" class="form-control p-1" placeholder="Ingrese la ruta">
								<div class="vacio oculto">¡Campo obligatorio!</div>
							</div>
						...
						<div class="row">
							<div class="form-group col-md-12">
								...
								<input name="descripcion" autocomplete="off" maxlength="200" type="text" class="form-control p-1 muestraMensaje" placeholder="Ingrese la descripción">
							</div> 
						</div>
					...
					listado+=`<tr id="${resp[i].ID_SUME}" >
									<td>
										<div class="estadoTachado nombre muestraMensaje ${mestado}">${resp[i].NOMB_SUME}</div>
									</td>
									<td>
										<div class="estadoTachado descripcion ${mestado}">${resp[i].DESC_SUME}</div>
									</td>
									<td>
	...
	let objeto={
		...
		ruta:$('#'+tabla+' input[name=ruta]'),
		orden:$('#'+tabla+' input[name=orden]'),
		descripcion:$('#'+tabla+' input[name=descripcion]'),
		tabla:tabla,
	}
	eventosSubmenu(objeto);
}

function eventosSubmenu(objeto){
	...
	$('#'+objeto.tabla+' div').on( 'click','button[name=btnGuarda]',function(){//guarda
		...
		validaFormularioSubmenu(objeto)
	});
	...
	$('#'+objeto.tabla+'Tabla tbody').on( 'click','td a.estado',function(){//estado
    	...
    	let id=evento.attr('id');
		let nombre=evento.find("td div.nombre").text();
		submenuEstado({id:id,nombre:nombre,tabla:objeto.tabla});
	});
	...
	$('#'+objeto.tabla+'Tabla tbody').on( 'click','td a.elimina',function(){//elimina
    	...
    	let id=evento.attr('id');
		let nombre=evento.find("td div.nombre ").text();
		submenuElimina({id:id,nombre:nombre,tabla:objeto.tabla});
	});
}

async function submenuEdita(objeto){
	...
	objeto.nombre.val(resp.NOMB_SUME);
	objeto.ruta.val(resp.RUTA_SUME);
	objeto.orden.val(resp.ORDE_SUME);
	objeto.descripcion.val(resp.DESC_SUME);
}

function validaFormularioSubmenu(objeto){
	validaVacio(objeto.nombre);
	validaVacio(objeto.orden);

	if(objeto.nombre.val()=="" || objeto.ruta.val()=="" || objeto.orden.val()==""){
		return false;
	}else{
		enviaFormularioSubmenu(objeto);
	}
}

function enviaFormularioSubmenu(objeto){
	...
	confirm("¡"+verbo+" el registro: "+dato+"!",function(){
		...
		try {
			...
			if(resp.resultado){
				if(objeto.id>0){
					$("#"+objeto.tabla+"Tabla #"+objeto.id+" .nombre").text(resp.info.nombreSubmenu);
					$("#"+objeto.tabla+"Tabla #"+objeto.id+" .descripcion").text((resp.info.descripcionSubmenu===null)?'':resp.info.descripcionSubmenu);
					...
					let rowNode =t.row.add( [
						`<div class="estadoTachado nombre muestraMensaje">${resp.info.nombreSubmenu}</div>`,
						`<div class="estadoTachado descripcion">${(resp.info.descripcionSubmenu===null)?'':resp.info.descripcionSubmenu}</div>`,
						estado()+modifica()+elimina()
					] ).draw( false ).node();
					$( rowNode ).attr('id',resp.info.idSubmenu);
...
function submenuElimina(objeto){
